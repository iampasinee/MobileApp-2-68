...
class DatabaseHelper {
  ...
  Future _onUpgrade(Database db, int oldVersion, int newVersion) async {
    if (oldVersion < 4) {
      ...
      await db.execute('ALTER TABLE tbUsers ADD COLUMN bmi_target REAL');
  ...
  Future<void> updateAllBmi() async {
    ...
    for (User user in users) {
      ...
      double target = User.getWeightAdvice(user.weight, user.height);

      await db.update(
        'tbUsers',
        {'bmi': bmi, 'bmi_type': bmiType, 'bmi_target': target},
        where: 'id = ?',
        whereArgs: [user.id],
      );
    }
